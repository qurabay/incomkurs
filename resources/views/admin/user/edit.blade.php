...
{{--        @endforeach--}}
        ...
        @foreach($groups as $group)
            <div>
                @php
                    $c = \App\Models\UserLesson::where('user_id',$user->id)->where('group_id', $group->id)->exists();
                @endphp

                <input {{$c ? 'checked':''}} id="group_{{$group->id}}" type="checkbox" name="user_groups[]" value="{{$group->id}}">
